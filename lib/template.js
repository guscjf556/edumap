...
module.exports ={
    ...
    create : function(){
      function imageFormMaker(){
        var imageForms = "";
        var categories = ["전체 <span class='badge badge-pill badge-danger '>필수</span>", "잎", "꽃", "줄기", "열매"];
        for(var i = 0; i < 5; i++){
          imageForms += `
            <div class="col text-center d-flex flex-column p-1">
              <input type="file" class="form-control-file d-none" id="o_image_${i}" name="o_image_${i}" accept="image/*" onchange="image${i}();" required>
              <label for="o_image_${i}" class="my-auto">
                <img id="imageResult${i}" src="/data/photoUpload.png" alt="" class="img-thumbnail">
              </label>
              <div class="font-weight-bold">${categories[i]}</div>
            </div>
          `
        }
        return imageForms
