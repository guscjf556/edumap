...
module.exports = {
  ...
  revise: function (pageId,queryResult) {
    function imageFormMaker() {
      ...
      imageForms += `
        <div>
            <div class="col text-center">
              <input type="file" class="form-control-file d-none" id="o_image_${1}" name="o_image_${1}" accept="image/*" onchange="image${1}();" required>
              <label for="o_image_${1}"><img id="imageResult${1}" src="${queryResult[0].o_image_1}" alt="" class="img-fluid img-thumbnail d-block" style="min-width:150px;height:150px;">${categories[0]}</label>
            </div>
          </div>
        `;
      for (var i = 2; i < 6; i++) {
        imageForms += `
          <div>
            <div class="col text-center">
              <input type="file" class="form-control-file d-none" id="o_image_${i}" name="o_image_${i}" accept="image/*" onchange="image${i}();" >
              <label for="o_image_${i}"><img id="imageResult${i}" src="${queryResult[0].o_image_}${i}" alt="추가" class="img-fluid img-thumbnail d-block" style="min-width:150px;height:150px;">${
          categories[i - 1]
        }</label>
            </div>
          </div>
    ...
    function setDefaultUploadImage(){
      ...
      for(let i = 2; i < 6; i++){
        code += `
        if(/undefined/.test(document.querySelector('#imageResult${i}').src)){
    ...
    return `
      ...
      var mapContainer = document.getElementById('map'), // 지도를 표시할 div 
          mapOption = { 
              center: new kakao.maps.LatLng(${queryResult[0].Lat}, ${queryResult[0].Lng}), // 지도의 중심좌표
              level: 3 // 지도의 확대 레벨
          };
